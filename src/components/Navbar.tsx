...
/**
 * Frosted sticky bar — wordmark left; section links, then a hairline divider
 * and GitHub / LinkedIn icons right. Links hide on small screens; icons stay.
 */
export function Navbar() {
  return (
    <header className="nav-frost sticky top-0 z-50 border-b border-rule">
          ...
          <ul className="hidden items-center gap-6 sm:flex">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="whitespace-nowrap font-body text-base leading-none text-muted transition-colors hover:text-ink"
                >
                  {link.label}
                </a>
              </li>
